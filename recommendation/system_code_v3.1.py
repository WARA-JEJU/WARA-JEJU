...
# 제주도 숙박 데이터
accom_path = './data/220124/_종합_숙박.xlsx'
...
def recommand_cafe(poi_df, select_id, past_pos):
    ...
    top20 = cafe_df[cafe_df['dist'] <= 15].sort_values('rating', ascending=True).iloc[:20, :]
...
accom_poi = pd.read_excel(accom_path, index_col=False)

# In[83]:


navi_map = None
if min_navi_vertexes is not None:
    navi_map = folium.Map(location=[jeju_pos[1], jeju_pos[0]], zoom_start=10)
    ...
    for p_id, pos in navi_points:
        ...
        temp = accom_poi.copy()
        temp['dist'] = cal_dist((x, y), (temp['x'], temp['y']))
        for idx, row in temp[temp['dist'] <= 3].iterrows():
            folium.Circle(location=[row['y'], row['x']], radius=10, popup=row['place_name']).add_to(navi_map)
        del temp
    folium.PolyLine([(default_pos[1], default_pos[0])] + min_navi_vertexes, color='red').add_to(navi_map)
else:
    print("경로 탐색 실패")
navi_map
...
straight_map = folium.Map(location=[jeju_pos[1], jeju_pos[0]], zoom_start=10)
...
for p_id, x, y in min_straight_points:
    ...
    temp['dist'] = cal_dist((x, y), (temp['x'], temp['y']))
    for idx, row in temp[temp['dist'] <= 3].iterrows():
        folium.Circle(location=[row['y'], row['x']], radius=10, popup=row['place_name']).add_to(straight_map)
    del temp
