        ...
        try:
            ...
            if select is not None or name is not None:
                ...
                def get_cosine_top5(original, category):
                    ...
                    # 유사도 결과 Top 5 반환
                    return original[original['input_sim'] > 0.2].sort_values(
                        ['input_sim', 'kakao_grade', 'kakao_review'],
                        ascending=False).iloc[:5, :]
...
def profile(request):
    ...
    jeju_poi[jeju_poi['상호명'].str.contains(place1)]
    place1_dt = jeju_poi[jeju_poi['상호명'].str.contains(place1)]
    A = place1_dt.iat[0, 1]
    B = place1_dt.iat[0,11]
    C = place1_dt.iat[0, 12]

    name = A
    grade = B
    review = C
    return render(request, 'customer/profile.html', {'name': name, 'grade': grade, 'review': review})
